...
2026-09-08 使用 Vivado 2023.2 XSim 对最终 RTL 运行 11 组行为级仿真，结果为 **11/11 PASS**。覆盖题目要求的基础密码锁功能，以及两项附加功能：KEY3 四位随机临时密码、FPGA 触发 Raspberry Pi 5 报警拍摄。
...
- `.wcfg`：Vivado 波形窗口配置；
- `.vcd`：标准波形交换文件；
- `.png`：从该次 Vivado XSim 生成的 VCD 渲染出的便于报告引用的总览图；
- `.log`：完整编译、展开、仿真日志；
...
脚本会为每个 testbench 建立独立的临时 Vivado 工程，避免污染正式 `.xpr`；随后运行 XSim、检查 `test_pass`、导出 WDB/VCD/WCFG/日志，并用 `render_vcd.py` 生成 PNG。默认 Python 路径为本机 `D:\anaconda\python.exe`，如在其他电脑运行，可在脚本中改为可用的 Python 3 与 Pillow 环境。
