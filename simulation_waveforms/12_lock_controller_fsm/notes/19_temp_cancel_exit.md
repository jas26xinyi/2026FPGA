# 19_temp_cancel_exit：TEMP 显示按 C 取消

窗口：**75620–75730 ns**，连续绝对时间；scenario=19。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/19_temp_cancel_exit_native.png)
...
KEY3→TEMP；C 的有效采样使 TEMP→WAIT。控制器不会因为退出显示就使 temporary_valid=0，它是来自临时密码生成器的输入。
...
| --- | --- | --- | --- |
| 75660 | `temporary_event 0→1` | 75665 | `state 1:WAIT/PASS→8:TEMP` |
| 75670 | `temporary_event 1→0` | 75675 | `state=8:TEMP` 保持 |
| 75680 | `key_valid 0→1`, `key_code A→C` | 75685 | `state 8:TEMP→1:WAIT/PASS`, 有效键码 `C` |
| 75690 | `key_valid 1→0` | 75695 | `state=1:WAIT/PASS` 保持, 按键有效位释放，不重复提交 |
...
配置：[WCFG](../configs/19_temp_cancel_exit.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/19_temp_cancel_exit.wcfg
