# 25_reset_from_3：从 ERROR(3) 同步复位并重新初始化

窗口：**87790–87940 ns**，连续绝对时间；scenario=25。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/25_reset_from_3_native.png)
...
测试平台经正常输入先到达 ERROR；前置建立过程保留在完整 VCD/WDB，当前图从该状态开始。
...
| --- | --- | --- | --- |
| 87830 | `rst 0→1`, `flash_init_done 1→0` | 87835 | `state 3:ERROR→0:BOOT`, `entry_digits 1111→0000`, `entry_count 4→0`, `error_count 1→0` |
| 87850 | `rst 1→0` | 87855 | `state=0:BOOT` 保持 |
| 87880 | `flash_init_done 0→1` | 87885 | `state 0:BOOT→1:WAIT/PASS` |
...
配置：[WCFG](../configs/25_reset_from_3.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/25_reset_from_3.wcfg
