# 04_user_timeout：用户输入无操作超时

窗口：**960–11070 ns**，连续绝对时间；scenario=4。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/04_user_timeout_native.png)
...
SW1→USER，输入 1 清零计时器。之后没有有效 activity；timer_count=998、999 时仍为 USER，下一上升沿在旧值 999 条件下 USER→WAIT，计时器和输入清零。完整图覆盖入口与等待；边界图放大最后数拍。
...
| --- | --- | --- | --- |
| 1000 | `sw1_event 0→1` | 1005 | `state 1:WAIT/PASS→2:USER` |
| 1010 | `sw1_event 1→0` | 1015 | `state=2:USER` 保持 |
| 1020 | `key_valid 0→1`, `key_code C→1` | 1025 | `state=2:USER` 保持, `entry_digits 0000→0001`, `entry_count 0→1`, 有效键码 `1` |
| 1030 | `key_valid 1→0` | 1035 | `state=2:USER` 保持, 按键有效位释放，不重复提交 |
| 11025 | 无新外部脉冲；计时或状态逻辑自然转移 | 11025 | `state 2:USER→1:WAIT/PASS`, `entry_digits 0001→0000`, `entry_count 1→0` |
...
配置：[WCFG](../configs/04_user_timeout.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/04_user_timeout.wcfg
