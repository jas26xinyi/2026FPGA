# 15_open_deadline_priority：OPEN 优先级：超时与 A 关锁均优先于 KEY1

窗口：**54660–74990 ns**，连续绝对时间；scenario=15。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/15_open_deadline_priority_native.png)
...
| --- | --- | --- | --- |
| 54700 | `sw1_event 0→1` | 54705 | `state 1:WAIT/PASS→2:USER` |
| 54710 | `sw1_event 1→0` | 54715 | `state=2:USER` 保持 |
| 54720 | `key_valid 0→1`, `key_code A→1` | 54725 | `state=2:USER` 保持, `entry_digits 0000→0001`, `entry_count 0→1`, 有效键码 `1` |
| 54730 | `key_valid 1→0` | 54735 | `state=2:USER` 保持, 按键有效位释放，不重复提交 |
| 54740 | `key_valid 0→1`, `key_code 1→2` | 54745 | `state=2:USER` 保持, `entry_digits 0001→0012`, `entry_count 1→2`, 有效键码 `2` |
| 54750 | `key_valid 1→0` | 54755 | `state=2:USER` 保持, 按键有效位释放，不重复提交 |
| 54760 | `key_valid 0→1`, `key_code 2→3` | 54765 | `state=2:USER` 保持, `entry_digits 0012→0123`, `entry_count 2→3`, 有效键码 `3` |
| 54770 | `key_valid 1→0` | 54775 | `state=2:USER` 保持, 按键有效位释放，不重复提交 |
| 54780 | `key_valid 0→1`, `key_code 3→4` | 54785 | `state=2:USER` 保持, `entry_digits 0123→1234`, `entry_count 3→4`, 有效键码 `4` |
| 54790 | `key_valid 1→0` | 54795 | `state=2:USER` 保持, 按键有效位释放，不重复提交 |
| 54800 | `key_valid 0→1`, `key_code 4→A` | 54805 | `state 2:USER→4:OPEN`, `unlocked 0→1`, 有效键码 `A` |
| 54810 | `key_valid 1→0` | 54815 | `state=4:OPEN` 保持, 按键有效位释放，不重复提交 |
...
配置：[WCFG](../configs/15_open_deadline_priority.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/15_open_deadline_priority.wcfg
