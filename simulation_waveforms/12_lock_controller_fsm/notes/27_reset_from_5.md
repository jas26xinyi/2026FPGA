# 27_reset_from_5：从 ADMIN/SET(5) 同步复位并重新初始化

窗口：**88390–88540 ns**，连续绝对时间；scenario=27。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/27_reset_from_5_native.png)
...
测试平台经正常输入先到达 ADMIN/SET；前置建立过程保留在完整 VCD/WDB，当前图从该状态开始。
...
| --- | --- | --- | --- |
| 88430 | `rst 0→1`, `flash_init_done 1→0` | 88435 | `state 5:ADMIN/SET→0:BOOT` |
| 88450 | `rst 1→0` | 88455 | `state=0:BOOT` 保持 |
| 88480 | `flash_init_done 0→1` | 88485 | `state 0:BOOT→1:WAIT/PASS` |
...
配置：[WCFG](../configs/27_reset_from_5.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/27_reset_from_5.wcfg
