# 30_reset_from_8：从 TEMP(8) 同步复位并重新初始化

窗口：**90240–90390 ns**，连续绝对时间；scenario=30。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/30_reset_from_8_native.png)
...
测试平台经正常输入先到达 TEMP；前置建立过程保留在完整 VCD/WDB，当前图从该状态开始。
...
| --- | --- | --- | --- |
| 90280 | `rst 0→1`, `flash_init_done 1→0` | 90285 | `state 8:TEMP→0:BOOT` |
| 90300 | `rst 1→0` | 90305 | `state=0:BOOT` 保持 |
| 90330 | `flash_init_done 0→1` | 90335 | `state 0:BOOT→1:WAIT/PASS` |
...
配置：[WCFG](../configs/30_reset_from_8.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/30_reset_from_8.wcfg
