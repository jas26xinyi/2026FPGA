# 24_reset_from_2：从 USER(2) 同步复位并重新初始化

窗口：**87440–87590 ns**，连续绝对时间；scenario=24。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/24_reset_from_2_native.png)
...
测试平台经正常输入先到达 USER；前置建立过程保留在完整 VCD/WDB，当前图从该状态开始。
...
| --- | --- | --- | --- |
| 87480 | `rst 0→1`, `flash_init_done 1→0` | 87485 | `state 2:USER→0:BOOT` |
| 87500 | `rst 1→0` | 87505 | `state=0:BOOT` 保持 |
| 87530 | `flash_init_done 0→1` | 87535 | `state 0:BOOT→1:WAIT/PASS` |
...
配置：[WCFG](../configs/24_reset_from_2.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/24_reset_from_2.wcfg
