# 28_reset_from_6：从 SAVE(6) 同步复位并重新初始化

窗口：**88740–88890 ns**，连续绝对时间；scenario=28。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/28_reset_from_6_native.png)
...
测试平台经正常输入先到达 SAVE；前置建立过程保留在完整 VCD/WDB，当前图从该状态开始。
...
| --- | --- | --- | --- |
| 88780 | `rst 0→1`, `flash_init_done 1→0` | 88785 | `state 6:SAVE→0:BOOT`, `entry_digits 4321→0000`, `entry_count 4→0`, `save_password 4321→1234` |
| 88800 | `rst 1→0` | 88805 | `state=0:BOOT` 保持 |
| 88830 | `flash_init_done 0→1` | 88835 | `state 0:BOOT→1:WAIT/PASS` |
...
配置：[WCFG](../configs/28_reset_from_6.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/28_reset_from_6.wcfg
