# 29_reset_from_7：从 ALARM(7) 同步复位并重新初始化

窗口：**89990–90140 ns**，连续绝对时间；scenario=29。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/29_reset_from_7_native.png)
...
测试平台经正常输入先到达 ALARM；前置建立过程保留在完整 VCD/WDB，当前图从该状态开始。
...
| --- | --- | --- | --- |
| 90030 | `rst 0→1`, `flash_init_done 1→0` | 90035 | `state 7:ALARM→0:BOOT`, `entry_digits 1111→0000`, `entry_count 4→0`, `error_count 4→0`, `alarm_active 1→0` |
| 90050 | `rst 1→0` | 90055 | `state=0:BOOT` 保持 |
| 90080 | `flash_init_done 0→1` | 90085 | `state 0:BOOT→1:WAIT/PASS` |
...
配置：[WCFG](../configs/29_reset_from_7.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/29_reset_from_7.wcfg
