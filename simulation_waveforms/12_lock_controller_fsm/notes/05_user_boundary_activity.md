# 05_user_boundary_activity：用户超时边界：有效数字优先，D 不算活动

窗口：**11070–21220 ns**，连续绝对时间；scenario=5。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/05_user_boundary_activity_native.png)
...
| --- | --- | --- | --- |
| 11110 | `sw1_event 0→1` | 11115 | `state 1:WAIT/PASS→2:USER` |
| 11120 | `sw1_event 1→0` | 11125 | `state=2:USER` 保持 |
| 11130 | `key_valid 0→1` | 11135 | `state=2:USER` 保持, `entry_digits 0000→0001`, `entry_count 0→1`, 有效键码 `1` |
| 11140 | `key_valid 1→0` | 11145 | `state=2:USER` 保持, 按键有效位释放，不重复提交 |
...
配置：[WCFG](../configs/05_user_boundary_activity.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/05_user_boundary_activity.wcfg
