# 07_admin_timeout：管理员输入超时不写 Flash

窗口：**21430–31540 ns**，连续绝对时间；scenario=7。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/07_admin_timeout_native.png)
...
WAIT；管理员使用同一 1000 拍输入门限。

## 当前激励与状态变化

KEY1→ADMIN，输入 5 后无操作。timer_count=999 后的上升沿 ADMIN→WAIT，save_request 全程为 0。完整图呈现入口/激励，边界图呈现触发条件。
...
| --- | --- | --- | --- |
| 21470 | `admin_event 0→1` | 21475 | `state 1:WAIT/PASS→5:ADMIN/SET` |
| 21480 | `admin_event 1→0` | 21485 | `state=5:ADMIN/SET` 保持 |
| 21490 | `key_valid 0→1`, `key_code C→5` | 21495 | `state=5:ADMIN/SET` 保持, `entry_digits 0000→0005`, `entry_count 0→1`, 有效键码 `5` |
| 21500 | `key_valid 1→0` | 21505 | `state=5:ADMIN/SET` 保持, 按键有效位释放，不重复提交 |
| 31495 | 无新外部脉冲；计时或状态逻辑自然转移 | 31495 | `state 5:ADMIN/SET→1:WAIT/PASS`, `entry_digits 0005→0000`, `entry_count 1→0` |
...
配置：[WCFG](../configs/07_admin_timeout.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/07_admin_timeout.wcfg
