# 26_reset_from_4：从 OPEN(4) 同步复位并重新初始化

窗口：**88140–88290 ns**，连续绝对时间；scenario=26。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/26_reset_from_4_native.png)
...
测试平台经正常输入先到达 OPEN；前置建立过程保留在完整 VCD/WDB，当前图从该状态开始。
...
| --- | --- | --- | --- |
| 88180 | `rst 0→1`, `flash_init_done 1→0` | 88185 | `state 4:OPEN→0:BOOT`, `entry_digits 1234→0000`, `entry_count 4→0`, `unlocked 1→0` |
| 88200 | `rst 1→0` | 88205 | `state=0:BOOT` 保持 |
| 88230 | `flash_init_done 0→1` | 88235 | `state 0:BOOT→1:WAIT/PASS` |
...
配置：[WCFG](../configs/26_reset_from_4.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/26_reset_from_4.wcfg
