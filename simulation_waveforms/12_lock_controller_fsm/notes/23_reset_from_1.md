# 23_reset_from_1：从 WAIT/PASS(1) 同步复位并重新初始化

窗口：**87190–87340 ns**，连续绝对时间；scenario=23。

## 原生 Vivado 截屏

![原生 Vivado 分段波形](../images/23_reset_from_1_native.png)
...
测试平台经正常输入先到达 WAIT/PASS；前置建立过程保留在完整 VCD/WDB，当前图从该状态开始。
...
| --- | --- | --- | --- |
| 87230 | `rst 0→1`, `flash_init_done 1→0` | 87235 | `state 1:WAIT/PASS→0:BOOT` |
| 87250 | `rst 1→0` | 87255 | `state=0:BOOT` 保持 |
| 87280 | `flash_init_done 0→1` | 87285 | `state 0:BOOT→1:WAIT/PASS` |
...
配置：[WCFG](../configs/23_reset_from_1.wcfg)，完整数据：[WDB](../tb_lock_controller_fsm.wdb)、[VCD](../tb_lock_controller_fsm.vcd)。
...
open_wave_config E:/26FPGA/simulation_waveforms/12_lock_controller_fsm/configs/23_reset_from_1.wcfg
